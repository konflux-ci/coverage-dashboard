...
func (c *Creator) commitChanges(ctx context.Context, configFile, repoFullName string) error {
	...
	commitMsg := fmt.Sprintf(`chore: add coverage tracking for %s

Add configuration for %s to the Konflux coverage dashboard.
This enables automatic test coverage tracking and reporting for the repository.`, repoFullName, repoFullName)
